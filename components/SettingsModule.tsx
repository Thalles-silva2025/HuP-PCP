...
import { Settings, Plus, Trash2, Database, Truck, Scissors, Box, Layers, Ruler, Tag, Save, Edit2, MapPin, Phone, Palette, StickyNote, Users, Power, X, ChevronDown, Check, AlertTriangle, Activity, Terminal } from 'lucide-react';
import { useToast } from '../contexts/ToastContext';
import { SystemLogService, SystemLog } from '../services/SystemLogService'; // IMPORTADO
...
export const SettingsModule: React.FC = () => {
  ...
  // ... (Keep existing handlers: handleAdd, handleAddColor, handleAddObs, requestDelete, confirmDelete, etc.)
  ...
  const handleAddVariant = () => {
      ...
      if (editingMaterial.variants?.some(v => v.name.toLowerCase() === variantInputName.toLowerCase())) { alert('Esta cor já foi adicionada.'); return; }
  ...
  const handleToggleMaterialStatus = async (material: Material) => { const newStatus = material.status === 'Ativo' ? 'Inativo' : 'Ativo'; await ApiService.saveMaterial({ ...material, status: newStatus }); setMaterials(await ApiService.getMaterials()); addToast({ type: 'info', title: 'Status Atualizado', message: `Material agora está ${newStatus}.` }); };
  ...
  const handleSavePartner = async (e: React.FormEvent) => {
      ...
      setPartners(updated); setIsPartnerModalOpen(false); setEditingPartner({}); addToast({ type: 'success', title: 'Salvo', message: 'Parceiro salvo com sucesso.' });
  };

  const openPartnerModal = (partner?: Partner) => { setEditingPartner(partner || { type: 'Facção', contractType: 'PJ' }); setIsPartnerModalOpen(true); };
  const handleSaveWarehouse = async (e: React.FormEvent) => { e.preventDefault(); if(!editingWarehouse.name) return; const updated = await ApiService.saveWarehouse(editingWarehouse as Warehouse); setWarehouses(updated); setIsWarehouseModalOpen(false); setEditingWarehouse({ type: 'Interno' }); addToast({ type: 'success', title: 'Salvo', message: 'Depósito salvo com sucesso.' }); };
          ...
          {activeTab === 'logs' && (
              <div className="animate-fade-in">
                  <div className="flex justify-between items-center mb-6 border-b pb-4">
                      ...
                      <div className="flex gap-2"><button onClick={loadLogs} className="text-gray-600 hover:text-blue-600 p-2"><Activity size={18}/></button><button onClick={handleClearLogs} className="text-red-500 hover:text-red-700 px-3 py-1 border border-red-200 rounded text-sm font-bold bg-red-50">Limpar Histórico</button></div>
                  </div>
                  {loadingLogs ? (
